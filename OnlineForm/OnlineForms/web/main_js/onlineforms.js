...
//    })
                        ...
                        success: function (data) {
                            ...
                            if (jsonvalue.hodDetails.bo == "no bo")
                            {
                                ...
                                if (jsonvalue.hodDetails.ophone !== "") {
                                    ...
                                    alert("Please update your RO details in LDAP then update your profile");
                                    $('#' + field_form + ' #hod_tel').val("");
                                    $('#' + field_form + ' #hod_tel').attr("readonly", false);
                                }
    ...
    $(document).on('change', '#hardware_cert_file', function (e) {
        //$('#hardware_cert_file').change(function (e) {

        e.preventDefault();
        var file = $('input[name="hardware_cert_file"]').get(0).files[0];
        var fileType = file.type;
        var fileSize = file.size;
        ...
        if (fileType === match[0]) {
            ...
            if (fileSize <= 1000000) {
                $('#file_err').text("");
                var formData = new FormData();
                formData.append('hardware_cert_file', file);
                formData.append('CSRFRandom', $('#CSRFRandom').val());
                formData.append('size', fileSize);
                $.ajax({
                    url: 'checkRelayHardwarePDF',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (data) {
                        ...
                        var jsonvalue = JSON.parse(myJSON);
                        if (jsonvalue.error !== '' && jsonvalue.error !== null) {
                            $('#hardware_cert_file').text(jsonvalue.error);
                            $('#hardwarecert').val('false');

                        } else {


                            $('#hardwarecert').val('true');

                            $('#hardware_cert_file3').text(jsonvalue.hardware_cert_fileFileName);
                ...
                $('#hardware_file_err').text("PDF file shoulb be less than 1mb");
            }
        } else {

            $('#hardware_file_err').text("Upload only PDF file");
        }
    });
...
//    });
    ...
    $(document).on('change', '#cert_file', function (e) {
        // alert("hhhhhhh");
        // $('#hardware_cert_file').change(function (e) {

        e.preventDefault();
        var field_form = $(this).closest('form').attr('id');
//        var file = $("#" + field_form + ' input[name="cert_file"]').get(0).files[0];
            ...
            if (fileSize <= 1000000) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var jsonvalue = JSON.parse(myJSON);
                        if (jsonvalue.error !== '' && jsonvalue.error !== null) {
                            $("#" + field_form + ' #cert_file').text(jsonvalue.error);
                            $("#" + field_form + ' #cert').val('false');
                            //  alert("inside if sahil");
                            // $("#"+field_form +' #uploaded_filename').text(jsonvalue.hardware_cert_fileFileName);
                        } else {
                            //   alert("inside else sahil");
                            $("#" + field_form + ' #cert').val('true');
                            $("#" + field_form + ' #uploaded_filename').text(jsonvalue.cert_fileFileName);
                        }
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.log('error');
                ...
                $("#" + field_form + ' #file_err').text("PDF file shoulb be less than 1mb");
            }
        } else {
            $("#" + field_form + ' #file_err').text("Upload only PDF file");
        }
    });
    ...
    $('#esign_submit').click(function () {
        ...
        if (esign === null) {
            ...
            $.ajax({
                type: "POST",
                url: "consent",
                data: {check: esign, formtype: formtype, CSRFRandom: csrf},
            ...
            if (esign === "manual_upload")
            {
                ...
                var msg = '<p>Please note, the form will remain pending at your end as long as you do not upload duly sealed and signed scanned copy of the application form. After uploading scanned copy your request will be automatically forwarded to the next level.</p>';
                // msg = +'<p>For other options, your request will be automatically forwarded to next level</p>';
                var csrf = $('#CSRFRandom').val();
                bootbox.confirm({
                    title: "DA Onboarding Submission",
                    message: msg,
                    buttons: {
                        confirm: {
                            label: 'Yes',
...
// $(document).on('click', '#delModal_yes', function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "CancelRequest",
            data: {data: value, CSRFRandom: csrf},
            success: function (data) {
                var myJSON = JSON.stringify(data);
                var jsonvalue = JSON.parse(myJSON);
    ...
    $(document).on('click', '.previewedit', function (e) // line modified by pr on 22ndmay18
    {
        ...
        var approve_btn = $(this).closest('tr').find('td:last').find('ul li:eq(1)').html();
        var reject_btn = $(this).closest('tr').find('td:last').find('ul li:eq(2)').html();
        var raise_btn = $(this).closest('tr').find('td:last').find('ul li:last').html();

        var ref_no = $(this).closest('tr').children('td.track_id:first').text();
        if (ref_no !== "")
        {
            if (!ref_no) {
        ...
        var csrf = $('#CSRFRandom').val();
//        var error = $('#error').val();
        var comingFrom = $("#comingFrom").val(); // 25 jan preeti
        // alert(comingFrom);
//        alert("1")
        $.ajax({
            type: "POST",
            url: "preview",
            data: {data: ref_no, whichform: whichform, comingFrom: comingFrom, CSRFRandom: csrf},
            dataType: 'json'
        }).done(function (data) {
            if (data.csrf_error) {
            ...
            showEdit = jsonvalue.isEditable;
            console.log("isEditable ::::::: " + showEdit);
            if (whichform === 'SMS') {
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData()
                                $(".preview-change .modal-title").html("Preview for " + ref_no);
...
//                                $('#sms_preview .edit').removeClass('display-hide');
                                ...
                                if (!showEdit)
                                    $("#sms_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#sms_preview .edit, .save-changes").removeClass('display-hide');
                ...
                if (jsonvalue.oldornew === 'old') {
                    $.ajax({
                        ...
                        if (dataResponse.trim() != '' && dataResponse != null) {
                            ...
                            if (!showEdit)
                                $("#dns_preview .edit, .save-changes").addClass('display-hide');
                            else if (showEdit)
                                $("#dns_preview .edit, .save-changes").removeClass('display-hide');
...
//                            $('#dns_preview .edit').removeClass('display-hide');
                            ...
                            $("#dns_preview .save-changes").hide();//26-04-2022 added by RAJESH   
...
//                            alert("2")    
                                validDomForReqModify2(jsonvalue.validRecord);
                                setTimeout(function () {
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit) {
                                    $("#wifi_preview .edit, .save-changes").addClass('display-hide');
                                } else if (showEdit) {
                                    $("#wifi_preview .edit, .save-changes").removeClass('display-hide');
                                }
                                ...
                                if (jsonvalue.prvw_wifitype === 'nic') {
                                    ...
                                    $('#wifi_preview #device_type1').val(jsonvalue.prvw_device_type1);
                                    if (jsonvalue.prvw_wifimac2 !== '' && jsonvalue.prvw_wifios2 !== '') {
                                        ...
                                        $('#wifi_preview #device_type2').val(jsonvalue.prvw_device_type2);
                                    } else {
                                    ...
                                    if (jsonvalue.prvw_wifimac3 !== '' && jsonvalue.prvw_wifios3 !== '') {
                                        ...
                                        $('#wifi_preview #device_type3').val(jsonvalue.prvw_device_type3);
                                    } else {
                                        $('#wifi_preview #wifi_mac3_div').addClass('display-hide');
                                    }
                                    //added by mohit sharma

                                    console.log("SAHIL......" + jsonvalue.prvw_device_type4);
                                    console.log("SAHIL......" + jsonvalue.prvw_device_type1);
                                    console.log("SAHIL......" + jsonvalue.prvw_device_type2);
                                    console.log("SAHIL......" + jsonvalue.prvw_device_type3);
                                    console.log("mohit......" + jsonvalue.prvw_wifimac4);
                                    console.log("mohit222......" + jsonvalue.prvw_wifios4);
                                    if (jsonvalue.prvw_wifimac4 !== '' && jsonvalue.prvw_wifios4 !== '') {
                                        $('#wifi_preview #wifi_mac4_div').removeClass('display-hide');
                                        $('#wifi_preview #wifi_mac4').val(jsonvalue.prvw_wifimac4);
                                        $('#wifi_preview #wifi_os4').val(jsonvalue.prvw_wifios4);
                                        $('#wifi_preview #device_type4').val(jsonvalue.prvw_device_type4);
                                    } else {
                                        $('#wifi_preview #wifi_mac4_div').addClass('display-hide');
                                    }
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $("#singleuser_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#singleuser_preview .edit, .save-changes").removeClass('display-hide');
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData()
                                if (!showEdit)
                                    $("#ldap_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#ldap_preview .edit, .save-changes").removeClass('display-hide');
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $('#imappop_preview .edit, .save-changes').addClass('display-hide');
                                else if (showEdit)
                                    $('#imappop_preview .edit, .save-changes').removeClass('display-hide');
...
//                                $('#imappop_preview .edit').removeClass('display-hide'); // commented by pr on 20thdec17, uncommented by pr on 29thjan18
            ...
            } else if (whichform === 'MOBILE' || whichform === 'PROFILE') {
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData()
                                if (!showEdit)
                                    $("#mobile_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#mobile_preview .edit, .save-changes").removeClass('display-hide');

                                if (jsonvalue.country_code == null)
                                {
                                    $('#mobile_preview #country_code').val(jsonvalue.country_code);
                                    $('#mobile_preview #country_code_tab').addClass('display-hide');
                                } else {

                                    $('#mobile_preview #country_code').val(jsonvalue.country_code);
                                    $('#mobile_preview #country_code_tab').removeClass('display-hide');
                                }
                                ...
                                $('#profile').html(jsonvalue.formType + " Update Form");
                                $('#profile1').html(jsonvalue.formType + " Update New Details");
                                $('#profile2').html(jsonvalue.formType + " Update Old Details");

                                $('#mobile_preview #nicDateOfBirth').val(jsonvalue.nicDateOfBirth);
                                $('#mobile_preview #nicDateOfRetirement').val(jsonvalue.nicDateOfRetirement);
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData()
                                if (!showEdit)
                                    $("#nkn_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#nkn_preview .edit, .save-changes").removeClass('display-hide');
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData()
                                if (!showEdit)
                                    $("#gem_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#gem_preview .edit, .save-changes").removeClass('display-hide');
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $("#bulkuser_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#bulkuser_preview .edit, .save-changes").removeClass('display-hide');
                                ...
                                if (jsonvalue.form_name == "BULKUSER")
                                {
                                    //alert(jsonvalue.form_name)
                                    $('#bulk_preview #upload_email').removeClass('display-hide');
                                    $("#bulk_preview #successBulkData thead tr").append("<td style='width: 6% !important;'>S.No</td><td>Domain</td><td>CNAME</td><td>New IP</td>");
                                    console.log(jsonvalue.validRecord);
                                    validDomForReqModify1(jsonvalue.validRecord);
                                    console.log(jsonvalue.validRecord);
                                    setTimeout(function () {
                                        $("#bulk_preview table#successBulkData button").prop('disabled', true);
                                    }, 10);
                                } else {
                                    $('#bulk_preview #upload_email').addClass('display-hide');
                                }
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData();
                                if (!showEdit)
                                    $("#distribution_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#distribution_preview .edit, .save-changes").removeClass('display-hide');
                                ...
                                $('#distribution_preview #memberCount').val(jsonvalue.memberCount);
                                ...
                                if (jsonvalue.moderator) {
                                    moderatorRecordTbl(jsonvalue.moderator);
                                }

                                if (jsonvalue.owner) {
                                    ownerRecordTbl(jsonvalue.owner);
                                }

//                                console.log("hhhh" + jsonvalue.moderator.length);
//                                alert(jsonvalue.moderator.length == 0);

                                if (jsonvalue.moderator.length == 0) {

                                    console.log("inside iffffffff ")
                                    $('#distribution_preview #moderator_collection').addClass("d-none");
                                    $('#distribution_preview #mod_heading').addClass("d-none");
                                }

                                if (jsonvalue.owner.length == 0) {

                                    $('#distribution_preview #owner_collection').addClass("d-none");
                                    $('#distribution_preview #own_heading').addClass("d-none");
                                }
...
//                                $('#distribution_preview .edit').removeClass('display-hide');
            ...
            } else if (whichform === 'BULKDLIST') {
                whichform = 'distribution';

                $.ajax({
                    type: "POST",
                    url: "bulk_distribution_preview",
                    dataType: 'html', complete: function () {
                        $('.loader').hide();
                    }
                })
                        .done(function (dataResponse) {
                            ...
                            console.log(jsonvalue.Excelfile);
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (jsonvalue.Excelfile !== undefined && jsonvalue.Excelfile !== null && jsonvalue.Excelfile) {
                                    $("#distribution_preview #own_heading_tbl tbody").html("");
                                    var tbody = '';

                                    $.each(jsonvalue.Excelfile, function (k, v) {

                                        var i = k + 1;
                                        tbody += "<tr><td > " + i + " </td><td > " + v.list_name + " </td><td> " + v.description_list + " </td><td> " + v.list_mod + " </td><td> " + v.allowed_member + " </td><td> " + v.list_temp + " </td><td> " + v.mail_Acceptance + " </td><td> " + v.owner_Name + " </td><td> " + v.Owner_Email + " </td><td> " + v.owner_Mobile + " </td><td> " + v.moderator_Name + " </td> <td> " + v.moderator_Email + " </td> <td> " + v.moderator_Mobile + " </td> <td> " + v.owner_Admin + " </td><td> " + v.moderator_Admin + " </td></tr>";

                                    });
                                    console.log(tbody)
                                    $("#own_heading_tbl #qwerty").html(tbody);
                                }
                                if (jsonvalue.moderator) {
                                    moderatorRecordTbl(jsonvalue.moderator);
                                }
...
//                                $('#distribution_preview .edit').removeClass('display-hide');
                                ...
                                $("#distribution_preview #tnc_div").addClass('display-hide');
                                if (curr_status === data.role) {
                                    createdombtn("#Dlist_bulk", approve_btn, reject_btn, raise_btn);
                                }
                ...
                var relay_url = 'relay_preview';
                if (jsonvalue.point_contact === undefined) {
                    relay_url = 'relay_preview_old';
                }
                $.ajax({
                    type: "POST",
                    url: relay_url,
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $("#relay_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#relay_preview .edit, .save-changes").removeClass('display-hide');
                                ...
                                if (jsonvalue.app_ip.indexOf(";") > -1) {
                                    ...
                                    for (var i = 0; i < myarray.length; i++)
                                    {
                                        ...
                                        if (i === 0) {
                                            ...
                                            var elmnt = '<div class="con mt-2"><input class="form-control class_name_prvw" placeholder="Enter the IP Address [e.g.: 127.0.0.1]" type="text" name="relay_ip[]" id="relay_ip1" value="' + service + '" maxlength="12"></div>';
                                ...
                                $('#relay_preview #req_').val(jsonvalue.req_type)
                                // alert(jsonvalue.req_type)

                                if (jsonvalue.req_type == "req_delete")
                                {
                                    $('#relay_preview #req_for_span').html("Delete")

                                }
                                if (jsonvalue.req_type == "req_new")
                                {
                                    $('#relay_preview #req_for_span').html("New")

                                }
                                if (jsonvalue.req_type == "req_add")
                                {
                                    $('#relay_preview #req_for_span').html("Add")

                                }
                                if (jsonvalue.req_type == "req_modify")
                                {
                                    $('#relay_preview #req_for_span').html("Modify")

                                }

                                if (jsonvalue.req_type == "req_new") {
                                    $('#relay_preview #req_new').attr('checked', true);

                                }
                                if (jsonvalue.req_type == "req_modify") {
                                    $('#relay_preview #req_modify').attr('checked', true);

                                }
                                if (jsonvalue.req_type == "req_add") {
                                    $('#relay_preview #req_add').attr('checked', true);

                                }
                                if (jsonvalue.req_type == "req_delete") {
                                    $('#relay_preview #req_delete').attr('checked', true);
                                    //$('#relay_preview #req_delete').attr('checked', true);


                                }

                                if (jsonvalue.req_type == "req_add" || jsonvalue.req_type == "req_modify")
                                {
                                    $("#relay_preview #oldip").removeClass('d-none');
                                    if (jsonvalue.old_app_ip.indexOf(";") > -1) {
                                        var myarray = jsonvalue.old_app_ip.split(';');
                                        for (var i = 0; i < myarray.length; i++)
                                        {
                                            var service = myarray[i].trim();
                                            if (i === 0) {

                                                $('#relay_preview #old_relay_ip').val(service);
                                            } else {
                                                var elmnt = '<div class="con mt-2"><input class="form-control" placeholder="Enter the IP Address [e.g.: 127.0.0.1]" type="text" name="old_relay_ip[]" id="old_relay_ip1" value="' + service + '" maxlength="12"></div>';
                                                $('#relay_preview #old_add_ip').append(elmnt);
                                            }
                                        }
                                    } else {

                                        $('#relay_preview #old_relay_ip').val(jsonvalue.old_app_ip);
                                        $("#relay_preview #oldip").addClass('d-none');
                                    }
                                }

                                if (jsonvalue.req_type !== "req_delete")
                                {
                                    $('#relay_preview #not-prompt-in-caseofdelete').removeClass("d-none")



                                    $('#relay_preview #mailsent').val(jsonvalue.relay_mailsent)


                                    // $('#relay_preview #relay_app_url').val(jsonvalue.relay_app_url);
                                    //  alert(jsonvalue.smtp_port)
                                    if (jsonvalue.smtp_port == "465") {
                                        $('#relay_preview #port_465').prop('checked', true);
                                        $("#relay_preview #auth_id_div").removeClass('d-none');
                                        $('#relay_preview #auth_id').val(jsonvalue.relay_auth_id);
                                    } else if (jsonvalue.smtp_port == "25") {
                                        $("#relay_preview #auth_id_div").addClass('d-none');
                                        $('#relay_preview #port_25').prop('checked', true);
                                    }



                                    if (jsonvalue.mail_type != null)
                                    {
                                        var myarray = jsonvalue.mail_type.split(',');


                                        if (myarray[0] === "OTP Service through Email") {
                                            $('#relay_preview #otp_mail_service').attr('checked', true);
                                        }

                                        if (myarray[1] === "Transactional Mails") {

                                            $('#relay_preview #mail_type_trans_mail').attr('checked', true);
                                        }

                                        if (myarray[2] === "Registration Mails") {
                                            $('#relay_preview #mail_type_reg_mail').attr('checked', true);
                                        }

                                        if (myarray[3] === "Forgot id/password") {
                                            $('#relay_preview #mail_type_forgotpass').attr('checked', true);
                                        }

                                        if (myarray[4] === "Alerts") {
                                            $('#relay_preview #mail_type_alert').attr('checked', true);
                                        }
                                        ...
                                        if (myarray[5] === "Others") {
                                            $('#relay_preview #mail_type_other').attr('checked', true);
                                            $("#relay_preview #other_mail_type").removeClass('d-none');
                                            $('#relay_preview #other_mail_type').val(myarray[6]);
                                        }
                                    ...
                                    if (jsonvalue.dkim == "dkim") {
                                        $('#relay_preview #dkimtest').attr('checked', true);
                                    } else {
                                        $('#relay_preview #dkimtest').attr('checked', false);
                                    }

                                    if (jsonvalue.spf == "spf") {
                                        $('#relay_preview #spftest').attr('checked', true);
                                    } else {
                                        $('#relay_preview #spftest').attr('checked', false);
                                    }

                                    if (jsonvalue.dmarc == "dmarc") {
                                        $('#relay_preview #dmarctest').attr('checked', true);
                                    } else {
                                        $('#relay_preview #dmarctest').attr('checked', false);
                                    }

//                              

                                    //$('#relay_preview #relay_sender_id').val(jsonvalue.relay_sender_id);
                                    $('#relay_preview #relay_sender_id').val(jsonvalue.relay_sender_id);
                                    $('#relay_preview #spftest').val(jsonvalue.spf);

                                    $('#relay_preview #dkimtest').val(jsonvalue.dkim);
                                    $('#relay_preview #dmarctest').val(jsonvalue.dmarc);
                                    $('#relay_preview #domain_mx').val(jsonvalue.domain_mx);
                                    //alert(jsonvalue.security_audit)
                                    //alert(jsonvalue.staging_ip)

                                    if (jsonvalue.security_audit === "Hardware" && jsonvalue.staging_ip === "yes") {
                                        $('#relay_preview #security_audit_1').attr('checked', true);
                                        $("#relay_preview #security_exp_date").addClass('d-none');
                                        $("#relay_preview #hardware_cert_div").addClass('d-none')
                                        $("#relay_preview #cert_div").addClass('d-none')
                                        $("#relay_preview #cert_div1").removeClass('d-none')
                                    }
                                    if (jsonvalue.security_audit === "Software" && jsonvalue.staging_ip === "yes") {
                                        $('#relay_preview #security_audit_2').attr('checked', true);
                                        $("#relay_preview #security_exp_date").addClass('d-none');
                                        $("#relay_preview #hardware_cert_div").addClass('d-none')
                                        $("#relay_preview #cert_div").addClass('d-none')
                                        $("#relay_preview #cert_div1").removeClass('d-none')
                                    ...
                                    if (jsonvalue.security_audit === "Hardware" && jsonvalue.staging_ip === "no") {

                                        $('#relay_preview #security_audit_1').attr('checked', true);
                                        $("#relay_preview #security_exp_date").addClass('d-none');
                                        $("#relay_preview #hardware_cert_div").removeClass('d-none')
                                        $("#relay_preview #cert_div").addClass('d-none')
                                        $("#relay_preview #cert_div1").addClass('d-none')
                                        if (jsonvalue.hardware_uploaded_filename !== "")
                                        {
                                            $('#relay_preview #hardware_uploaded_filename').text(jsonvalue.hardware_uploaded_filename);
                                        }
                                    }

                                    if (jsonvalue.security_audit === "Software" && jsonvalue.staging_ip === "no") {

                                        $('#relay_preview #security_audit_2').attr('checked', true);
                                        $("#relay_preview #security_exp_date").removeClass('d-none');
                                        $("#relay_preview #security_exp_date").val(jsonvalue.security_exp_date)
                                        $("#relay_preview #hardware_cert_div").addClass('d-none')
                                        $("#relay_preview #cert_div").removeClass('d-none')
                                        $("#relay_preview #cert_div1").addClass('d-none')
                                        if (jsonvalue.uploaded_filename !== "")
                                        {
                                    ...
                                    if (jsonvalue.point_contact == "yes") {
                                        $('#relay_preview #point_contact_1').attr('checked', true);
                                    }
                                    if (jsonvalue.point_contact == "no") {
                                        $('#relay_preview #point_contact_2').attr('checked', true);
                                    }

                                    if (jsonvalue.domain_mx === "mailgw.nic.in")
                                    {

                                        $('#relay_preview #other-record-addition').addClass('d-none')

                                    } else {
                                        $('#relay_preview #other-record-addition').removeClass('d-none')

                                    }
//                                if (jsonvalue.is_hosted === "yes") {
//                                    $('#relay_preview #is_hosted_1').attr('checked', true);
//                                }
//
//                                if (jsonvalue.is_hosted === "no") {
//                                    $('#relay_preview #is_hosted_2').attr('checked', true);
//
//                                }
                                    $('#relay_preview #security_audit').val(jsonvalue.security_audit);

                                    $('#relay_preview #other_mail_type').val(jsonvalue.other_mail_type);
                                    $('#relay_preview #point_contact').val(jsonvalue.point_contact);



                                } else {

                                    $('#relay_preview #not-prompt-in-caseofdelete').addClass("d-none")
                                }
                                // $('#')
                                $('#relay_preview #relay_app_url').val(jsonvalue.relay_app_url);
...
//                                $('#relay_preview .edit').removeClass('display-hide');
                                ...
                                $('#relay_preview #point_name').val(jsonvalue.point_name);
                                $('#relay_preview #point_email').val(jsonvalue.point_email);
                                $('#relay_preview #mobile_number').val(jsonvalue.mobile_number);
                                $('#relay_preview #landline_number').val(jsonvalue.landline_number);
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $("#ip_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#ip_preview .edit, .save-changes").removeClass('display-hide');
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $('#webcast_preview .edit, .save-changes').addClass('display-hide');
                                else if (showEdit)
                                    $('#webcast_preview .edit, .save-changes').removeClass('display-hide');
...
//                                $('#webcast_preview .edit').removeClass('display-hide'); // commented by pr on 20thdec17, uncommented by pr on 29thjan18
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                loadFormData()
                                if (!showEdit)
                                    $("#centralutm_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $('#centralutm_preview .edit, .save-changes').removeClass('display-hide');
            ...
            } else if (whichform === 'WIFIPORT') {

                $.ajax({
                    type: "POST",
                    url: "wifiport_preview",
                    dataType: 'html', complete: function () {
                        $('.loader').hide();
                    }
                })
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                $('.wifiport_preview').html(dataResponse);
                                $(".preview-change .modal-title").html("Preview for " + ref_no);
                                loadFormData()

//                                  if (!showEdit) // to show /hide edit on preview based on stat_type
//                                {
//                                    $("#wifiport_preview .edit,.save-changes").hide();
//                                } else if (showEdit) {
//                                    // start, code added by pr on 25thjan18                            
//                                    if (comingFrom === 'admin')
//                                    {
//                                        $("#wifiport_preview .edit").addClass('display-hide');
//                                        $('#wifiport_preview .save-changes').addClass('display-hide'); // line added by pr on 20thdec17                                                            
//                                    } else
//                                    {
//                                        $("#wifiport_preview .edit,.save-changes").show();
//                                    }
//                                    // end, code added by pr on 25thjan18
//                                }
                                if (!showEdit)
                                    $("#wifiport_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                                    $("#wifiport_preview .edit, .save-changes").removeClass('display-hide');
                                $('#wifiport_preview #add_sourceip').html('');

                                $('#wifiport_preview #add_designationip').html('');

                                $('#wifiport_preview #add_service').html('');

                                $('#wifiport_preview #add_port').html('');

                                $('#wifiport_preview #add_action').html('');

                                $('#wifiport_preview #add_time').html('');
                                ...
                                if (jsonvalue.sourceIP.indexOf(";") > -1) {
                                    ...
                                    for (var i = 0; i < myarray.length; i++)
                                    {
                                        ...
                                        if (i === 0) {
                                            ...
                                            $('#wifiport_preview #wifiport_sourceip_1').val(sourceIP);
                                        } else {

                                            //var elmnt = '<div class="con"><input class="form-control dns_name" placeholder="Enter the DNS URL [e.g.: https://demo.nic.in or https://demo.gov.in]" type="text" name="dns_url[]" value="' + service + '" maxlength="100" style="margin: 20px 0px 20px 15px; width: 87%;"><span id="domain1" style="position: relative;width: 65px;float: right;margin-right: 52px;margin-top: -54px;height: 34px;padding: 6px 12px;border: 1px solid #c2cad8;">.' + jsonvalue.dns_type + '.in</span>' + '<button type="button" class="rmvbox1" style="position: relative;float: right;margin-top: -49px;font-size: xx-large;background: none;border: none;" title="Remove Row" id="dns_rmv1" name="dns_rmv1"><i class="fa fa-remove"></i></button></div>';
                                            var elmnt = '<div class="con"><input class="form-control dns_name class_name_url1" placeholder="Enter the Source IP [e.g.: demo.nic.in or demo.gov.in]" type="text" name="wifiport_sourceip[]" value="' + sourceIP + '" maxlength="100" style="margin: 20px 0px 20px 0px; "></div>';
                                            $('#wifiport_preview #add_sourceip').append(elmnt);
                                        }
                                    }
                                } else {


                                    $('#wifiport_preview #wifiport_sourceip_1').val(jsonvalue.sourceIP);
                                }
                                ...
                                if (jsonvalue.destinationIP.indexOf(";") > -1) {
                                    ...
                                    for (var i = 0; i < myarray.length; i++)
                                    {
                                        ...
                                        if (i === 0) {
                                            ...
                                            $('#wifiport_preview #wifiport_destinationip_1').val(destinationIP);
                                        } else {

                                            //var elmnt = '<div class="con"><input class="form-control dns_name" placeholder="Enter the DNS URL [e.g.: https://demo.nic.in or https://demo.gov.in]" type="text" name="dns_url[]" value="' + service + '" maxlength="100" style="margin: 20px 0px 20px 15px; width: 87%;"><span id="domain1" style="position: relative;width: 65px;float: right;margin-right: 52px;margin-top: -54px;height: 34px;padding: 6px 12px;border: 1px solid #c2cad8;">.' + jsonvalue.dns_type + '.in</span>' + '<button type="button" class="rmvbox1" style="position: relative;float: right;margin-top: -49px;font-size: xx-large;background: none;border: none;" title="Remove Row" id="dns_rmv1" name="dns_rmv1"><i class="fa fa-remove"></i></button></div>';
                                            var elmnt = '<div class="con"><input class="form-control dns_name class_name_url1" placeholder="Enter the Destination IP [e.g.: demo.nic.in or demo.gov.in]" type="text" name="wifiport_destinationip[]" value="' + destinationIP + '" maxlength="100" style="margin: 20px 0px 20px 0px; "></div>';
                                            $('#wifiport_preview #add_designationip').append(elmnt);
                                        }
                                    }
                                } else {

                                    $('#wifiport_preview #wifiport_destinationip_1').val(jsonvalue.destinationIP);
                                }
                                    ...
                                    if (jsonvalue.service.indexOf(";") > -1) {
                                        ...
                                        for (var i = 0; i < myarray.length; i++)
                                        {
                                            ...
                                            if (i === 0) {
                                                ...
                                                $('#wifiport_preview #wifiport_service_1').val(service);
                                            } else {
                                                var tcp_selected = "", udp_selected = "", icmp_selected = "";
                                                if (service === "tcp")
                                                {
                                                ...
                                                var elmnt = '<div class="con"><select class="form-control ser" id="" name="wifiport_service[]" style="margin: 20px 0px 20px 0px; ">' +
                                                        "<option value=''>-SELECT-</option><option value='tcp' " + tcp_selected + ">TCP</option>" +
                                                        "<option value='udp' selected ='" + udp_selected + "' " + udp_selected + ">UDP</option>" +
                                                        "<option value='icmp' selected ='" + icmp_selected + "' " + icmp_selected + ">ICMP</option>" +
                                                        "</select></div>";
                                                $('#wifiport_preview #add_service').append(elmnt);
                                            }
                                        }
                                    } else {

                                        $('#wifiport_preview #wifiport_service_1').val(jsonvalue.service);
                                    }
                                }
                                ...
                                if (jsonvalue.ports.indexOf(";") > -1) {
                                    ...
                                    for (var i = 0; i < myarray.length; i++)
                                    {
                                        var ports = myarray[i].trim();
                                        if (i === 0) {
                                            $('#wifiport_preview #wifiport_port_1').val(ports);
                                        } else {
                                            var elmnt = '<div class="con"><input class="form-control class_name_loc1" placeholder="Enter the wifiport port" value="' + ports + '" name="wifiport_port[]" id="dns_loc" maxlength="100" style="margin: 20px 0px 20px 0px;"></div>';
                                            $('#wifiport_preview #add_port').append(elmnt);
                                        }
                                    }
                                } else {

                                    $('#wifiport_preview #wifiport_port_1').val(jsonvalue.ports);
                                }
                                ...
                                if (jsonvalue.action.indexOf(";") > -1) {
                                    ...
                                    for (var i = 0; i < myarray.length; i++)
                                    {
                                        var action = myarray[i].trim();
                                        if (i === 0) {
                                            $('#wifiport_preview #wifiport_action_1').val(action);
                                        } else {
                                            ...
                                            var elmnt = '<div class="con"><select class="form-control ser" id="" name="wifiport_action[]" style="margin: 20px 0px 20px 0px; ">' +
                                                    "<option value=''>-SELECT-</option><option value='permit' " + permit_selected + ">Permit</option>" +
                                                    "<option value='deny' " + deny_selected + ">Deny</option>" +
                                                    "</select></div>";
                                            $('#wifiport_preview #add_action').append(elmnt);
                                    ...
                                    $('#wifiport_preview #wifiport_action_1').val(jsonvalue.action);
                                }
                                ...
                                if (jsonvalue.timeperiod.indexOf(";") > -1) {
                                    ...
                                    for (var i = 0; i < myarray.length; i++)
                                    {
                                        var timeperiod = myarray[i].trim();
                                        if (i === 0) {
                                            $('#wifiport_preview #wifiport_timePeriod_1').val(timeperiod);
                                        } else {

                                            var elmnt = '<div class="con"><input class="form-control class_name_loc1" placeholder="Enter the time period" value="' + timeperiod + '" name="wifiport_time[]" id="dns_loc" maxlength="100" style="margin: 20px 0px 20px 0px; width:85%"></div>';
                                            $('#wifiport_preview #add_time').append(elmnt);
                                            //$('#wifiport_preview #add_time').append(elmnt);
                                        }
                                    }
                                } else {

                                    $('#wifiport_preview #wifiport_timePeriod_1').val(jsonvalue.timeperiod);
                                }



                                $('#wifiport_preview #remarks').val(jsonvalue.prvw_purpose);

                                $('#wifiport_preview_tab').find('input, textarea, button, select').attr('disabled', 'true');

                                $("#wifiport_preview :button[type='button']").removeAttr('disabled');

                                $("#wifiport_preview #tnc").removeAttr('disabled');

                                $("#wifiport_preview #tnc_div").addClass('display-hide');

                                if (curr_status === data.role) {

                                    createdombtn("#wifiport_preview", approve_btn, reject_btn, raise_btn);
                                }

                                $('#wifiport_preview_form').modal({backdrop: 'static', keyboard: false});
                            }
                        });
            } else if (whichform === 'VPN' || whichform === 'VPNRENEW' || whichform === 'VPNMOD' || whichform === 'VPNADD' || whichform === 'VPNSURRENDER' || whichform === 'VPNDELETE') { //added by nikki on 26th jun 2019
                whichform = 'vpn';
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $('#vpn_preview .edit, .save-changes').addClass('display-hide');
                                else if (showEdit)
                                    $('#vpn_preview .edit, .save-changes').removeClass('display-hide');
                                ...
                                $('#vpn_preview #vpn_coo').val(jsonvalue.prvw_vpncoord);
                                if (jsonvalue.user_type !== "vpn_renew") {

                                    var content = "<table class='table table-bordered table-hover'>"

//                                   if(ref_no.toString().indexOf("MOD") > -1 )
//                                   {
                                    content += '<th>IP Type</th><th>IP Address</th><th>Application URL</th><th>Destination Port</th><th>Server Location</th><th>Action Type</th>';
//                                    }else{
//                                        
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $("#email_act_preview .edit, .save-changes").addClass('display-hide');
                                else if (showEdit)
                    ...
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                if (!showEdit)
                                    $('#email_deact_preview .edit, .save-changes').addClass('display-hide');
                                else if (showEdit)
                                    $('#email_deact_preview .edit, .save-changes').removeClass('display-hide');
...
//                                $('#email_deact_preview .edit').removeClass('display-hide');
            ...
            } else if (whichform === "DOREXT") { //added by sahil 
                whichform = 'dor_ext';
                $.ajax({
                    type: "POST",
                    url: "dor_ext_preview",
                    dataType: 'html', complete: function () {
                        ...
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                ...
                                $('.dor_ext_preview').html(dataResponse);
                                $(".preview-change .modal-title").html("Preview for " + ref_no);
                                loadFormData()

                                $('#dor_ext_preview .edit, .save-changes').addClass('display-hide');
//                                if (!showEdit)
//                                    $('#dor_ext_preview .edit, .save-changes').addClass('display-hide');
//                                else if (showEdit)
//                                    $('#dor_ext_preview .edit, .save-changes').removeClass('display-hide');

                                if (jsonvalue.single_id_type === 'id_name')
                                {

                                    $('#dor_ext_preview #single_id_type_dor_1').prop('checked', true);
                                }
                                if (jsonvalue.single_id_type === 'id_desig')
                                {

                                    $('#dor_ext_preview #single_id_type_dor_2').prop('checked', true);
                                }

                                if (jsonvalue.single_emp_type === 'emp_regular') {

                                    $('#dor_ext_preview #single_emp_type_dor1').prop('checked', true);
                                }
                                if (jsonvalue.single_emp_type === 'consultant') {
                                    $('#dor_ext_preview #single_emp_type_dor2').prop('checked', true);
                                }
                                if (jsonvalue.single_emp_type === 'emp_contract') {
                                    $('#dor_ext_preview #single_emp_type_dor3').prop('checked', true);

                                }

                                if (jsonvalue.single_emp_type == "emp_regular") {

                                    $('#dor_ext_preview #fileUploadDiv').addClass("d-none");
                                    $('#dor_ext_preview #fileDownloadDiv').addClass("d-none");
                                } else {
                                    $('#dor_ext_preview #fileUploadDiv').removeClass("d-none");
                                    $('#dor_ext_preview #fileDownloadDiv').removeClass("d-none");
                                }

//                                $('#dor_ext_preview .edit').removeClass('display-hide');
                                $('#dor_ext_preview_tab').find('input, textarea, button, select').attr('disabled', 'true');
                                $("#dor_ext_preview :button[type='button']").removeAttr('disabled');
                                $("#dor_ext_preview #tnc").removeAttr('disabled');
                                $("#dor_ext_preview #tnc_div").addClass('display-hide');
                                if (curr_status === data.role) {
                                    createdombtn("#dor_ext_preview", approve_btn, reject_btn, raise_btn);
                                }

                                $('#dor_ext_preview #dor_email').val(jsonvalue.dor_email);
                                $('#dor_ext_preview #email_single_dob2').val(jsonvalue.single_dob);

                                $('#dor_ext_preview #p_email_single_dor').val(jsonvalue.pre_dor);
                                $('#dor_ext_preview #email_single_dor2').val(jsonvalue.dor);
                                $('#dor_ext_preview #hardware_cert_file3').text(jsonvalue.uploaded_filename);


                                $('#dor_ext_preview_form').modal({backdrop: 'static', keyboard: false});
                            } else {
                                console.log('Preview is not loading');
                            }
                        })
            ...
            } else if (whichform === "DOREXTRT") { //added by sahil 
                whichform = 'dor_ext_retired';
                $.ajax({
                    type: "POST",
                    url: "dor_ext_preview_retired",
                    dataType: 'html', complete: function () {
                        $('.loader').hide();
                    }
                })
...
//                                $('#dor_ext_preview .edit').removeClass('display-hide');
                                ...
                                $('#dor_ext_preview #dor_email').val(jsonvalue.dor_email);
                                $('#dor_ext_preview #email_single_dob2').val(jsonvalue.single_dob);
                                $('#dor_ext_preview #p_email_single_name').val(jsonvalue.prvw_offname)
                                $('#dor_ext_preview #p_email_mobile_dor').val(jsonvalue.prvw_mobile)

                                $('#dor_ext_preview #p_email_single_dor').val(jsonvalue.pre_dor);
                                $('#dor_ext_preview #p_email_single_newdor').val(jsonvalue.dor);
                                $('#dor_ext_preview #hardware_cert_file3').text(jsonvalue.uploaded_filename);
            ...
            } else if (whichform === 'DAONBOARDING') {
                $.ajax({
                    type: "POST",
                    url: "daonboarding_preview",
                    dataType: 'html', complete: function () {
                        $('.loader').hide();
                    }
                })
                        .done(function (dataResponse) {
                            ...
                            if (dataResponse.trim() != '' && dataResponse != null) {
                                $('.daonboarding_preview').html(dataResponse);
                                $(".modal-title").html("Preview for " + ref_no);
                                loadFormData()

                                if (!showEdit)
                                    $('#daonboarding_preview .edit, .save-changes').addClass('display-hide');
                                else if (showEdit)
                                    $('#daonboarding_preview .edit, .save-changes').removeClass('display-hide');

                                $('#daonboarding_preview #uploaded_filename').text(jsonvalue.prvw_uploaded_filename);

                                $('#daonboarding_preview #bo_name').val(jsonvalue.prvw_bo_name);
                                $('#daonboarding_preview  #vpn_reg_no').val(jsonvalue.prvw_vpn_reg_no);
                                if (jsonvalue.daon_mobile === 'on')
                                    $('#daonboarding_preview #daon_mobile').prop("checked", true);
                                else
                                    $('#daonboarding_preview #daon_mobile').prop("checked", false);

                                if (jsonvalue.eligibility === 'department') {
                                    $('#daonboarding_preview #eligibility_department').prop("checked", true);
                                    $('#daonboarding_preview #eligibility_psu').prop("checked", false);

                                } else {
                                    $('#daonboarding_preview #eligibility_department').prop("checked", false);
                                    $('#daonboarding_preview #eligibility_psu').prop("checked", true);
                                }

//                                $('#daonboarding_preview .edit').removeClass('display-hide');
                                $('#daonboarding_preview_tab').find('input, textarea, button, select').attr('disabled', 'true');
                                $("#daonboarding_preview :button[type='button']").removeAttr('disabled');
                                $("#daonboarding_preview #tnc").removeAttr('disabled');
                                $("#daonboarding_preview #tnc_div").addClass('display-hide');
                                if (curr_status === data.role) {
                                    createdombtn("#daonboarding_preview", approve_btn, reject_btn, raise_btn);
                                }
                                $('#daonboarding_preview_form').modal({backdrop: 'static', keyboard: false});
                                $("#daonboarding_preview :input ").prop('readonly', true);
            ...
            function loadFormData() {
                if (whichform.toLowerCase() == 'profile') {
                    whichform = 'mobile'
                }
...
$(document).on('click', '.fupload_1', function () {
    ...
    $('#form_val').val(whichform)
    $('#ref_no').val(ref_no)
    if (whichform == "DAONBOARDING") {
        $("#fileUpload #da-onboarding-manual-upload-note").removeClass("text-hide");
    }
                    ...
                    success: function (data) {
                        ...
                        if (jsonvalue.error !== '' && jsonvalue.error !== null) {
                            ...
                            $.ajax({
                                ...
                                } else if (whichform === "DOREXT") { //added by nikki on 26th jun 2019
                                    whichform = 'dor_ext';
                                    $.ajax({
                                        type: "POST",
                                        url: "dor_ext_preview",
                                        dataType: 'html', complete: function () {
                                            ...
                                            .done(function (dataResponse) {
                                                ...
                                                if (dataResponse.trim() != '' && dataResponse != null) {
                                                    ...
                                                    if (!showEdit) // to show /hide edit on preview based on stat_type
                                                    {
                                                        $("#dor_ext_preview .edit,.save-changes").hide();
                                                    } else if (showEdit) {
                                                        // start, code added by pr on 25thjan18    

                                                        if (comingFrom === 'admin')
                                                        {
                                                            $("#dor_ext_preview .edit").addClass('display-hide');
                                                            $('#dor_ext_preview .save-changes').addClass('display-hide'); // line added by pr on 20thdec17                                                            
                                                        } else
                                                        {

                                                            $("#dor_ext_preview .edit").removeClass('display-hide');
                                                            $('#dor_ext_preview .save-changes').removeClass('display-hide');
                                                            //$("#email_act_preview .edit,.save-changes").removeClass('display-hide');
                                                    ...
                                                    $('#dor_ext_preview #dor_email').val(jsonvalue.dor_email);
                                                    $('#dor_ext_preview #p_email_single_dor').val(jsonvalue.pre_dor);
                                                    $('#dor_ext_preview #email_single_dor1').val(jsonvalue.dor);
                                                    $('#dor_ext_preview #uploaded_filename').text(jsonvalue.uploaded_filename);
...
$(document).on('click', '.fupload', function () {
    ...
    window.daonboarding_ca_upload_approve_element = $(this).parent().siblings().find(":contains(' Approve ')");
    ...
    if (fileType === match[0]) {
        ...
        if (fileSize <= 1000000) {
            ...
            $.ajax({
                ...
                dataType: 'html',
                success: function (data, datasuc, data2) {
                    console.log("success");
                    ...
                    bootbox.alert({
                        message: msg,
                        callback: function () {
                            if (panel == "CA" && whichform == "DAONBOARDING") {
                                window.daonboarding_ca_upload_approve_element.click();
...
function fdownload(ref_no) {
    ...
    $("#fileDownload .modal-title").html("File Download (" + ref_no + ")");
    //var ref_no = $(this).closest('tr').children('td:first').text();
    var whichform = ref_no.substring(0, ref_no.indexOf('-'));
    var showEdit = $(this).attr("id"); // to show /hide edit on preview based on stat_type
    $('#form_val').val(whichform)
    ...
    formData.append('CSRFRandom', CSRFRandom);
...
$(document).on('blur', '#newmobile', function (e) {
    ...
    $.ajax({
        ...
        success: function (data)
        {
            ...
            } else if (jsonvalue.returnString.indexOf("There is already an email address") > -1)
            {
                $("#new-code").addClass("display-hide");
                $('#moberr').html(jsonvalue.returnString);// text change 20thmay2020
            } else {
    ...
    $.ajax({
        ...
        success: function (data)
        {
            ...
            } else if (jsonvalue.returnString == "mobilesameus")
            {
                ...
                $("#newcode").val('');
            } else if (jsonvalue.returnString == "notexistprofile")
            {
...
function approve_ca(reg_no, form_name, stat_type)
{
    ...
    $.ajax({
        ...
        success: function (data)
        {
            ...
            if (jsonvalue.response === "profile_err" && jsonvalue.response_emp === "emp_code_err")
            {
                bootbox.dialog({
                    message: "<p style='text-align: center; font-size: 18px;'>Please go to profile and fill your profile first<br/></p>",
                    title: "",
                    buttons: {
                        cancel: {
                            label: "OK",
            ...
            } else if (jsonvalue.response === "profile_err" && jsonvalue.response_emp !== "emp_code_err")
            {
                bootbox.dialog({
                    message: "<p style='text-align: center; font-size: 18px;'>Please go to profile and fill your profile first<br/></p>",
                    title: "",
            ...
            {
                bootbox.dialog({
                    message: "<p style='text-align: center; font-size: 18px;'>Please go to preview/edit and fill your employee code<br/></p>",
                    title: "",
                    buttons: {
                        cancel: {
                            label: "OK",
...
function createdombtn(popup_id, approve_btn, reject_btn, raise_btn) {
    $('.preview-change .modal-footer').prepend(`<div class = "special_button_collection" >
            ...
            <span class = "btn btn-secondary req_raise ml-1" > ` + raise_btn + ` </span>
            </div>`);
...
//Line Added by Rahul jan 2021

$('#user_file_dlist').change(function (e) {
    e.preventDefault();
    // alert("okk in user");
    var file = $('input[name="user_file"]').get(0).files[0];
    var fileType = file.type;
    var fileSize = file.size
    // var match = ["text/xlsx", "application/vnd.ms-excel"]; // for multiple take comma separated values
    var match = ["text/xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]

    //alert(fileType+"---"+match[0]);
    ...
    if (fileType === match[0] || fileType === match[1]) {
        ...
        $('#file_err').text("Upload file in .xlsx format");
    }
});


function generatePdfs(regNo) {
    $.ajax({
        type: "POST",
        url: "generatefile",
        data: {data: regNo},
        ...
        success: function (data) {
            ...
            if (!data.Blob) {
                ...
                link.download = regNo + ".pdf";
                link.click();
            } else {
                bootbox.alert('You are not authorized to generate the PDF for this request as this request is still pending with RO/FO/Nodal Officer.');
            }
        }, error: function (data) {
...
//reader.readAsText(data.responseText);
//             var myJSON = JSON.stringify(data);
//             console.log(myJSON);
//                    var jsonvalue = JSON.parse(myJSON);
...
//                $('.loader').hide();
//            bootbox.alert('You are not authorized to generate the PDF for this request as this request is still pending with RO/FO/Nodal Officer.');
            var error = "It seems your IP has been changed. Kindly refresh and try again!!!"
            $('#new_alert .modal-body').html(error);
            $('#new_alert').modal('show');
            $('.loader').hide();
        }
