...
public class SingleApplicationContextSlave {

    private final String ctxfactory = ServletActionContext.getServletContext().getInitParameter("ctx-factory");
    private final String providerurl = ServletActionContext.getServletContext().getInitParameter("provider-url-slave");
    private final String nic_bo = ServletActionContext.getServletContext().getInitParameter("nic_bo");
    private final String binddn = ServletActionContext.getServletContext().getInitParameter("bind-dn");
//    private final String bindpass = ServletActionContext.getServletContext().getInitParameter("createpass");

    Base64.Decoder decoder = Base64.getDecoder();
	//private final String bindpass = new String(decoder.decode(ServletActionContext.getServletContext().getInitParameter("bind-pass")));
	private final String bindpass = ServletActionContext.getServletContext().getInitParameter("bind-pass");
    private Hashtable ht = new Hashtable();
    private static DirContext ctx = null;
    
    
    private SingleApplicationContextSlave()
    {
    ...
    public static DirContext getLDAPInstance()
    {
        if( ctx == null )
        {
            new SingleApplicationContextSlave();
        }
