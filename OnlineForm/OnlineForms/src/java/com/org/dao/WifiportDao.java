...
public class WifiportDao {

    private Connection con = null;
    private Connection conSlave = null;
    String ip = ServletActionContext.getRequest().getRemoteAddr();

    public void Wifiport_tab1(List<FormBean> valueObj, String purpose) {
        String sourceip = "";
        ...
        try {
            ...
            String sql = "insert into onlineform_temp_db.wifiport_entries(sourceIP,destinationIP,service,ports,action,timePeriod,purpose)values (?,?,?,?,?,?,?)";
    ...
    public String Wifiport_tab2(FormBean form_details, Map profile_values) {
        ...
        try {
            ...
            String search_regnum = "select registration_no from wifiport_registration where date(datetime)=date(now()) group by registration_no desc limit 1";
            ...
            //13,15
            newref = "WIFIPORT-FORM" + pdate1 + newref;
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == REF NUMBER DNS dao tab 2: " + newref);

            String sql = "insert into wifiport_registration (auth_email,mobile,auth_off_name,ophone,rphone,designation,emp_code,address,city,add_state,pin,employment,ministry,department,other_dept,state,organization,hod_name,hod_email,hod_mobile,hod_telephone,registration_no,ca_desig,purpose) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == wifiport dao tab 2: " + pst);
            ...
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == exception in wifiport dao tab 2: " + e.getMessage());
        } finally {
            try {
                if (pst != null) {
                    pst.close();
    ...
    public boolean WifiportAllowedUsers(Set<String> aliases) {
        ...
        try {
            ...
            pst = conSlave.prepareStatement(search_regnum);
            System.out.println("wifiportAllowedUsers@@@@@@@@@@@@@@@" + pst);
            rs1 = pst.executeQuery();
            if (rs1.next()) {
                errflag = true;
            }
        ...
        System.out.println("in dao wifiportAllowedUsers errflag" + errflag);
