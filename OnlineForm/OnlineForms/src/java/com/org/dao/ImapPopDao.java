...
public class ImapPopDao {
    ...
    public void IMAPPOP_tab1(FormBean form_details) {
        ...
        try {
            ...
            // get bean of profile
            String sql = "insert into onlineform_temp_db.imappop_registration_tmp (protocol,ip) values (?,?)";
            pst = con.prepareStatement(sql);
            pst.setString(1, form_details.getProtocol());
            pst.setString(2, ip);
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == " + "IMAP dao tab 1 :" + pst);
            pst.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == " + "exception in IMAP dao tab 1: " + e.getMessage());
        } finally {
            try {
                if (pst != null) {
                    pst.close();
    ...
    public String IMAPPOP_tab2(FormBean form_details, Map profile_values) {
        ...
        try {
            ...
            String search_regnum = "select registration_no from imappop_registration where date(datetime)=date(now()) group by registration_no desc limit 1";
            ...
            String sql = "insert into imappop_registration (auth_email,mobile,auth_off_name,ophone,rphone,designation,emp_code,address,city,add_state,pin,employment,ministry,department,"
                    + "other_dept,state,organization,hod_name,hod_email,hod_mobile,hod_telephone,protocol,registration_no,userip,ca_desig) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            pst = con.prepareStatement(sql);
            ...
            pst.setString(22, form_details.getProtocol());
            pst.setString(23, newref);
            pst.setString(24, ip);
            pst.setString(25, form_details.getCa_design().trim());
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == " + "IMAP dao tab 2 :" + pst);
            int i = pst.executeUpdate();
        } catch (Exception e) {
            errflag = true;
            newref = "";
            // e.printStackTrace();
            System.out.println(ServletActionContext.getRequest().getSession().getId() + " == " + "exception in IMAP dao tab 2: " + e.getMessage());
        } finally {
            try {
                if (pst != null) {
                    pst.close();
